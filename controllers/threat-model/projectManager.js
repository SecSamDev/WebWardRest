...
exports.findThreatModel = async (req, res, next) => {
    ...
    try {
        ...
        let userInProject = await db.query(`
            SELECT 
                *
            FROM web_projects
            INNER JOIN users
                ON web_projects.project_manager=users.manager
            WHERE
                users.id=$1 AND
                web_projects.id=$2
        `, [req.user.id, req.params.proj]);
        if (userInProject.rowCount < 1)
    ...
    if (req.body !== null && req.body !== undefined && req.params && typeof req.params.proj === 'string') {
        try {
            ...
            `, [name, web_project, description, applicationType,
                    owners, authors, stakeholders, url, system, platform]);
            status = 200;
            data = {};
