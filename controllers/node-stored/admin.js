...
exports.findAllNodes = async (req, res, next) => {
    ...
    try {
        let resDB = await db.query(`
            SELECT node_stored.id,node_stored.name
            FROM node_stored
            INNER JOIN
                pipelines
                    ON node_stored.pipe=pipelines.id
            WHERE 
                pipelines.owner=$1
        `, [req.user.id]);
        if (resDB.rows.length < 1)
...
exports.createNode = async (req, res, next) => {
    ...
    if (req.body !== null && req.body !== undefined) {
        try {
            ...
            let nodeX = req.body.x ? req.body.x : 20;
            let nodeY = req.body.y ? req.body.y : 20; 
            ...
            let dbresult = await db.query(`
                INSERT INTO node_stored 
                    (name,tag,type,owner,data,status,x,y) 
                VALUES 
                    ($1,$2,$3,$4,$5,$6,$7,$8)
                RETURNING *;
            `, [name, tag, type, req.user.id, data2, sts, nodeX, nodeY]);
