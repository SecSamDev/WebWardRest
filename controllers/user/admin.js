    ...
    if (req.params != null && req.params !== undefined
        ...
        try {
            ...
            if (dbresult.rows.length === 1) {
                ...
                let password = (req.body.password && typeof req.body.password === 'string') ? await db.hash(req.body.password) : dbresult.rows[0].password;
                if (manager) {
                ...
                let dbSave = await db.query(`
                    UPDATE users 
                    SET email=$2,role=$3,manager=$4,password=$5
                    WHERE 
                    id=$1 AND (manager=$6 OR $1=$6)
                `, [dbresult.rows[0].id, email, role, manager,password, req.user.id]);
                status = 200;
                data = {};
            } else {
                throw new Error("Data not found");
...
exports.deleteUser = async (req, res, next) => {
    ...
    let data = { message: 'Data not valid' };
    if (req.params !== null && req.params !== undefined) {
        if (req.params.name && typeof req.params.name === 'string' && req.params.name !== 'admin') {
            try {
                let resDB = await db.query(`
                DELETE FROM 
                    users 
                WHERE 
                    id=$1
                    AND (role !=3 OR $2='admin')
                `, [req.params.name,req.user.username]);
                if (resDB.rowCount >= 1) {
                    status = 200;
                    data = {};
                }
